...
@http.require_GET
def gallery(request):
    bats = models.Species.objects.select_related("genus").all()
    return render(request, "bats/gallery.html", {"bats": bats})


@http.require_GET
def detail(request, slug: str):
    bat_attributes = models.SpeciesAttributes.objects.filter(
        language=translation.get_language())
    bat_red_book = models.SpeciesRedBook.objects.filter(
        language=translation.get_language())

    prefetch_species_attributes = Prefetch(
        'species_attributes', queryset=bat_attributes)
    prefetch_bat_red_book = Prefetch('species_red_book', queryset=bat_red_book)

    bat = models.Species.objects.select_related("genus").prefetch_related(
        'species_images', prefetch_species_attributes, prefetch_bat_red_book).get(slug=slug)
