...
class LoginForm(forms.Form):
    username = forms.CharField(label=_("Username"), max_length=100, widget=forms.TextInput(
...
class BatSpeciesCreateForm(forms.ModelForm):
    is_red_book = forms.BooleanField(label=_(
        'Is it a Red Book specie?'), widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}))
    name = forms.CharField(label=_('Name'), widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': _('Name')}))
    genus = forms.ModelChoiceField(label=_('Genus'), initial=_('Select Genus'), widget=forms.Select(
        attrs={'class': 'form-select'}), queryset=batModels.Genus.objects.all())
    cover_image = forms.ImageField(label=_("Cover Image"), widget=forms.ClearableFileInput(
        attrs={'multiple': False, 'class': 'form-control'}), required=False)
...
class SpeciesAttributesForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ("description", "language", "distribution",
                  "biology", "conservation", "habitat", "threats")
...
class SpeciesRedBookForm(forms.ModelForm):
    ...
    description = forms.CharField(label=_(
        "Description"), widget=ckeditor_widgets.CKEditorUploadingWidget(), required=False)
...
SpeciesAttributesFormset = forms.inlineformset_factory(
    batModels.Species, batModels.SpeciesAttributes, form=SpeciesAttributesForm, max_num=len(settings.LANGUAGES), can_delete=True)
SpeciesImageFormset = forms.inlineformset_factory(
    batModels.Species, batModels.SpeciesImage, form=SpeciesImageForm, extra=5, max_num=10, can_delete=True)
SpeciesRedBookFormset = forms.inlineformset_factory(
    batModels.Species, batModels.SpeciesRedBook, form=SpeciesRedBookForm, max_num=len(settings.LANGUAGES), can_delete=True)
...
AuthorAttributesFormset = forms.inlineformset_factory(
    baseModels.Author, baseModels.AuthorAttributes, form=AuthorAttributesForm, max_num=len(settings.LANGUAGES), can_delete=True)


class ArticleForm(forms.ModelForm):
    name = forms.CharField(label=_("Name"), widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': _('Name')}))
    url = forms.URLField(label=_("Link"), widget=forms.URLInput(
        attrs={'class': 'form-control', 'placeholder': _('Link')}))
    author = forms.CharField(label=_("Author"), widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': _('Author')}))
...
ProjectAttributesFormset = forms.inlineformset_factory(
    activityModels.Project, activityModels.ProjectAttributes, form=ProjectAttributesForm, max_num=len(settings.LANGUAGES), can_delete=True)
ProjectImageFormset = forms.inlineformset_factory(
    activityModels.Project, activityModels.ProjectImage, form=ProjectImageForm, extra=5, max_num=10, can_delete=True)
...
SiteVisitAttributesFormset = forms.inlineformset_factory(
    activityModels.SiteVisit, activityModels.SiteVisitAttributes, form=SiteVisitAttributesForm, max_num=len(settings.LANGUAGES), can_delete=True)
SiteVisitImageFormset = forms.inlineformset_factory(
    activityModels.SiteVisit, activityModels.SiteVisitImage, form=SiteVisitImageForm, extra=5, max_num=10, can_delete=True)
